  ...
  try {
    init.customCommandRegistry.registerCommand(
      ...
      (origin) => {
        ...
        // ── Kill switch check ──
        if (!isStoreEnabled()) {
          system.run(() => player.sendMessage("§8[§cStore§8]§c Fitur sedang dinonaktifkan admin. Coba lagi nanti."));
          return;
        }

        if (!checkCooldown(player)) {
          system.run(() => player.sendMessage("§8[§cStore§8]§c Tunggu sebentar!"));
          return;
        }
        ...
        system.run(async () => {
          try { await openStoreMenu(player); }
          catch (e) { if (!e?.isUIClose) console.warn("[Store]", e); }
