...
import { displayName, enchantSummary, giveItem, giveItemFromListing, removeCachedClone, getCategory, rawItemName } from "../utils/items.js";
...
async function uiListingDetail(player, listingId) {
  while (true) {
    ...
    if (!l) { player.sendMessage("§8[§cAuction§8]§c Listing tidak ditemukan/sudah expired."); return; }
    const name = displayName(l.itemData);
    const qty = l.itemData.amount > 1 ? ` x${l.itemData.amount}` : "";
...
async function executeBuyout(buyer, listingId) {
  const result = await withLock(buyer.id, () => {
    ...
    if (!setCoin(buyer, getCoin(buyer) - l.price)) {
      clearTx(buyer.id);
      console.error(`[Auction] buyout aborted: setCoin(buyer) failed ${buyer.name} ${l.price}`);
      return { ok: false, err: "internal" };
    }
    try { pointActivity(buyer); } catch {}

    // Refund buyout offer
    if (l.offerId && l.offerAmount > 0) {
      const bidder = world.getPlayers().find(p => p.id === l.offerId);
      if (bidder) addCoin(bidder, l.offerAmount);
      else { pushNotif(l.offerId, `§8[§aAuction§8]§a Tawaran kamu §e${fmt(l.offerAmount)} Koin §adikembalikan.`); addPendingCoin(l.offerId, l.offerAmount); }
    }
    // Refund auction bidder
    if (l.mode === "auction" && l.bidderId && l.currentBid > 0) {
      const bidder = world.getPlayers().find(p => p.id === l.bidderId);
      if (bidder) { addCoin(bidder, l.currentBid); bidder.sendMessage(`§8[§eAuction§8]§e Item di-buyout! §f${fmt(l.currentBid)} Koin §edikembalikan.`); }
      else { pushNotif(l.bidderId, `§8[§eAuction§8]§e Item di-buyout. §f${fmt(l.currentBid)} Koin §edikembalikan.`); addPendingCoin(l.bidderId, l.currentBid); }
    }

    const gave = giveItemFromListing(buyer, listingId, l.itemData);
    if (!gave) { addPendingItem(buyer.id, l.itemData); removeCachedClone(listingId); }
    const sellFee = calcFee(l.price, l.sellerName);
    const sellerPayout = l.price - sellFee;
    const seller = world.getPlayers().find(p => p.id === l.sellerId);
    if (seller) {
      // [§2] Seller payout to online player. If addCoin fails (scoreboard error),
      // fall back to pending so coin is not destroyed.
      if (addCoin(seller, sellerPayout)) {
        playSfx(seller, SFX.SOLD);
        seller.sendMessage(`§8[§aAuction§8]§a Item §f${displayName(l.itemData)} §aterjual!\n§8  Pembeli: §f${buyer.name}\n§8  Harga : §e${fmt(l.price)} Koin\n§8  Fee   : §c-${fmt(sellFee)} Koin\n§8  Terima: §a${fmt(sellerPayout)} Koin`);
      } else {
        addPendingCoin(l.sellerId, sellerPayout);
        console.warn(`[Auction] seller payout fallback to pending: ${l.sellerName} ${sellerPayout}`);
        pushNotif(l.sellerId, `§8[§aAuction§8]§a Item §f${displayName(l.itemData)} §aterjual §e${fmt(l.price)} Koin §a(fee §c-${fmt(sellFee)}§a) oleh §f${buyer.name}§a! §7Klaim manual.`);
      }
  ...
  buyer.sendMessage(`§8[§aAuction§8]§a Pembelian berhasil!\n§8  Item  : §f${result.itemName}\n§8  Harga : §e${fmt(result.price)} Koin` + (!result.gave ? "\n§c⚠ Inventory penuh! Item masuk pending." : ""));
  if (result.price >= CFG.BROADCAST_MIN_PRICE) {
    const enchBadge = result.hasEnch ? " §dEnch" : "";
    world.sendMessage(`§8[§6Auction§8]§6 §e${buyer.name} §fmembeli §e${result.itemName}${enchBadge} §fdari §e${result.sellerName} §fseharga §e${fmt(result.price)} Koin§f!`);
  }
}
...
async function uiPlaceBid(buyer, listingId) {
  ...
  if (!bidOptions.length) {
    buyer.sendMessage(`§8[§cAuction§8]§c Saldo tidak cukup untuk bid! Minimal §f${fmt(minBid - myEscrow)} Koin.`);
    return;
  }
  ...
  const result = await withLock(buyer.id, () => {
    ...
    if (fresh.bidderId && fresh.currentBid > 0 && fresh.bidderId !== buyer.id) {
      ...
      const itemName = displayName(fresh.itemData);
      if (oldBidder) {
        addCoin(oldBidder, fresh.currentBid);
        playSfx(oldBidder, SFX.OUTBID);
        oldBidder.sendMessage(`§8[§cAuction§8]§c §fKamu di-outbid! §e${itemName} §fsekarang §e${fmt(amount)} Koin§f. §a${fmt(fresh.currentBid)} Koin §fdikembalikan.`);
      } else {
        pushNotif(fresh.bidderId, `§8[§cAuction§8]§c §fDi-outbid! §e${itemName} §fsekarang §e${fmt(amount)} Koin§f. §a${fmt(fresh.currentBid)} Koin §fdikembalikan.`);
        addPendingCoin(fresh.bidderId, fresh.currentBid);
      }
    }

    // Deduct only the difference (actualCost) — no need to refund+recharge.
    // [§2 iron rule] Atomic deduct — abort if scoreboard write fails.
    writeTx(buyer.id, { type: "bid", listingId, amount });
    if (!setCoin(buyer, getCoin(buyer) - actualCost)) {
      clearTx(buyer.id);
      console.error(`[Auction] bid aborted: setCoin(buyer) failed ${buyer.name} ${actualCost}`);
      return { ok: false, err: "internal" };
    }
    try { pointActivity(buyer); } catch {}
  ...
  if (result === false) { buyer.sendMessage("§8[§cAuction§8]§c Coba lagi sebentar."); return; }
  if (!result.ok) {
    if (result.err === "outbid") buyer.sendMessage("§8[§cAuction§8]§c Sudah ada bid lebih tinggi! Coba lagi.");
    else if (result.err === "internal") buyer.sendMessage("§8[§cAuction§8]§c Bid gagal, coba lagi sebentar.");
    else buyer.sendMessage("§8[§cAuction§8]§c Listing tidak tersedia / saldo kurang.");
    return;
  }
  playSfx(buyer, SFX.BID);
  if (myEscrow > 0) {
    buyer.sendMessage(`§8[§aAuction§8]§a Bid dinaikkan ke §b${fmt(amount)} Koin§a! Tambahan: §e${fmt(result.actualCost)} Koin`);
  } else {
    buyer.sendMessage(`§8[§aAuction§8]§a Bid §b${fmt(amount)} Koin §aberhasil dipasang!`);
  }
  const seller = world.getPlayers().find(p => p.id === l.sellerId);
  if (seller) {
    playSfx(seller, SFX.BID);
    seller.sendMessage(`§8[§bAuction§8]§b Bid baru!\n§8  Item : §f${displayName(l.itemData)}\n§8  Dari : §f${buyer.name}\n§8  Bid  : §b${fmt(amount)} Koin`);
  } else { pushNotif(l.sellerId, `§8[§bAuction§8]§b Bid baru §b${fmt(amount)} Koin §edari §f${buyer.name} §euntuk §f${displayName(l.itemData)}§e!`); }

  // Bid war broadcast — excite other players to join
  const freshAfter = getActiveListings().find(x => x.id === listingId);
  if (freshAfter && (freshAfter.bidCount || 0) > 1) {
    const enchBadge = l.itemData.enchantments?.length > 0 ? " §dEnch" : "";
    world.sendMessage(`§8[§6Auction§8] §bBid war! §e${displayName(l.itemData)}${enchBadge} §8sudah §e${freshAfter.bidCount} bidder §8- §e${fmt(amount)} Koin §8saat ini!`);
  }
}
...
async function uiMakeOffer(buyer, listingId) {
  ...
  if (!Number.isFinite(amount) || amount < minOffer) { buyer.sendMessage(`§8[§cAuction§8]§c Minimal §f${fmt(minOffer)} Koin.`); return; }
  if (amount >= l.price) { buyer.sendMessage("§8[§cAuction§8]§c Tawaran harus di bawah harga listing."); return; }
  if (getCoin(buyer) < amount) { buyer.sendMessage("§8[§cAuction§8]§c Saldo tidak cukup!"); return; }
  ...
  const result = await withLock(buyer.id, () => {
    ...
    if (fresh.offerId && fresh.offerAmount > 0 && fresh.offerId !== buyer.id) {
      ...
      if (oldBidder) { addCoin(oldBidder, fresh.offerAmount); oldBidder.sendMessage("§8[§eAuction§8]§e Tawaranmu digantikan. Koin dikembalikan."); }
      else { pushNotif(fresh.offerId, `§8[§eAuction§8]§e Tawaranmu §e${fmt(fresh.offerAmount)} Koin §edigantikan.`); addPendingCoin(fresh.offerId, fresh.offerAmount); }
    }
    if (fresh.offerId === buyer.id && fresh.offerAmount > 0) addCoin(buyer, fresh.offerAmount);
    // [§2] Atomic deduct — abort if scoreboard write fails.
    if (!setCoin(buyer, getCoin(buyer) - amount)) {
      console.error(`[Auction] offer aborted: setCoin(buyer) failed ${buyer.name} ${amount}`);
      return { ok: false, err: "internal" };
    }
    try { pointActivity(buyer); } catch {}
  ...
  if (result === false) { buyer.sendMessage("§8[§cAuction§8]§c Coba lagi sebentar."); return; }
  if (!result.ok) {
    if (result.err === "internal") buyer.sendMessage("§8[§cAuction§8]§c Tawaran gagal, coba lagi sebentar.");
    else buyer.sendMessage("§8[§cAuction§8]§c Listing tidak tersedia / saldo kurang.");
    return;
  }
  playSfx(buyer, SFX.OFFER);
  buyer.sendMessage(`§8[§aAuction§8]§a Tawaran §e${fmt(amount)} Koin §aterkirim!`);
  const seller = world.getPlayers().find(p => p.id === l.sellerId);
  if (seller) {
    playSfx(seller, SFX.OFFER);
    seller.sendMessage(`§8[§eAuction§8]§e Tawaran baru!\n§8  Item  : §f${displayName(l.itemData)}\n§8  Dari  : §f${buyer.name}\n§8  Harga : §b${fmt(amount)} Koin\n§8  Buka Listing Saya untuk merespons.`);
  } else { pushNotif(l.sellerId, `§8[§eAuction§8]§e Tawaran baru §b${fmt(amount)} Koin §edari §f${buyer.name} §euntuk §f${displayName(l.itemData)}§e!`); }
}
