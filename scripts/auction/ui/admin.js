...
import { displayName, giveItem, giveItemFromListing, removeCachedClone, itemIcon } from "../utils/items.js";
...
export async function uiAdmin(player) {
  if (!player.hasTag(CFG.ADMIN_TAG)) { player.sendMessage("§8[§cAuction§8]§c Akses ditolak."); return; }
...
async function adminRemoveListing(admin) {
  ...
  if (l.offerId && l.offerAmount > 0) {
    ...
    if (bidder) { addCoin(bidder, l.offerAmount); bidder.sendMessage(`§8[§eAuction§8]§e Admin menghapus listing. §f${fmt(l.offerAmount)} Koin §edikembalikan.`); }
    else {
      pushNotif(l.offerId, `§8[§eAuction§8]§e Admin menghapus listing. §f${fmt(l.offerAmount)} Koin §edikembalikan.`);
  ...
  if (l.mode === "auction" && l.bidderId && l.currentBid > 0) {
    ...
    if (bidder) { addCoin(bidder, l.currentBid); bidder.sendMessage(`§8[§eAuction§8]§e Admin menghapus lelang. §f${fmt(l.currentBid)} Koin §edikembalikan.`); }
    else {
      pushNotif(l.bidderId, `§8[§eAuction§8]§e Admin menghapus lelang. §f${fmt(l.currentBid)} Koin §edikembalikan.`);
  ...
  if (seller) {
    const gave = giveItemFromListing(seller, l.id, l.itemData);
    if (!gave) { addPendingItem(seller.id, l.itemData); removeCachedClone(l.id); }
    seller.sendMessage(`§8[§cAuction§8]§c Admin menghapus listing §f${displayName(l.itemData)}§c. Item dikembalikan.`);
  } else {
    addPendingItem(l.sellerId, l.itemData);
    removeCachedClone(l.id);
    pushNotif(l.sellerId, `§8[§cAuction§8]§c Admin menghapus listing §f${displayName(l.itemData)}§c. Item dikembalikan.`);
  }

  removeListing(l.id);
  playSfx(admin, SFX.ADMIN);
  admin.sendMessage(`§8[§aAuction Admin§8]§a Listing §f${displayName(l.itemData)} §a(${l.sellerName}) dihapus.`);
}
...
async function adminSetFee(admin) {
  ...
  if (!Number.isFinite(newFee) || newFee < 0 || newFee > 30) {
    admin.sendMessage("§8[§cAuction§8]§c Nilai tidak valid."); return;
  }

  settings.feePct = newFee;
  saveSettings(settings);
  playSfx(admin, SFX.ADMIN);
  admin.sendMessage(`§8[§aAuction Admin§8]§a Fee diubah ke §f${newFee}%`);
}
  ...
  if (!hist.length) {
    ...
    for (let i = 0; i < hist.length; i++) {
      ...
      if (h.type === "sold" || h.type === "offer_accepted") {
        ...
        body += `\n§f${i + 1}. §8[§8Expired§8]§8 §7${h.seller}\n`;
...
async function adminDPStats(admin) {
  while (true) {
    ...
    if (result && (result.players > 0 || result.keys > 0)) {
      admin.sendMessage(`§8[§aDP Cleanup§8]§a Dibersihkan §f${result.keys} §akeys dari §f${result.players} §aplayer inaktif.`);
    } else {
      admin.sendMessage("§8[§7DP Cleanup§8]§7 Tidak ada data player inaktif untuk dibersihkan.");
    }
  }
}
