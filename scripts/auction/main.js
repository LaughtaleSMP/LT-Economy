...
import {
  ...
  recoverTx, calcFee, getHistory,
} from "./utils/storage.js";
import { displayName, giveItem, giveItemFromListing, returnItemToOwner, removeCachedClone, cleanupCloneCache } from "./utils/items.js";
import {
  getCoin, addCoin, fmt, playSfx, timeAgo,
...
export async function openAuctionNPC(player) {
  if (isPurgeActive()) {
    player.sendMessage("§8[Sistem] §cPilar Energi offline — auction tidak tersedia.");
    return;
  }
  if (!checkCooldown(player)) {
    player.sendMessage("§8[§cAuction§8]§c Tunggu sebentar!");
    return;
  }
  await openAuction(player);
}
...
async function _menuLoop(player) {
  ...
  while (true) {
    ...
    form.button(`§b  Log Transaksi\n§r  §8Riwayat jual beli terbaru`, "textures/items/book_writable");
    btns.push("log");
    ...
    switch (btns[res.selection]) {
      ...
      case "log":    await showHistory(player);   break;
...
function claimPending(player) {
  ...
  if (pending.length > 0) {
    ...
    if (claimed > 0) player.sendMessage(`§8[§aAuction§8]§a §f${claimed} item pending §adiklaim!`);
  }
  const pendCoin = claimPendingCoin(player, addCoin);
  if (pendCoin > 0) {
    player.sendMessage(`§8[§aAuction§8]§a §e${fmt(pendCoin)} Koin §aditerima dari penjualan!`);
  }
}
...
async function showHistory(player) {
  ...
  let body = `${CFG.HR}\n`;
  body += `§b  ★ L O G   T R A N S A K S I\n`;
  body += `${CFG.HR}\n`;

  if (!hist.length) {
    body += `\n  §8Belum ada riwayat transaksi.\n`;
  } else {
    const recent = hist.slice(0, 15);
    body += `\n  §8${recent.length} transaksi terbaru:\n\n`;
    for (const h of recent) {
      if (h.type === "sold" || h.type === "offer_accepted") {
        body += `  §a${h.seller} §8-> §b${h.buyer}\n`;
        body += `  §8  ${h.item} §8-- §e${fmt(h.price)} Koin\n`;
      } else if (h.type === "auction_won") {
        body += `  §a${h.seller} §8-> §b${h.buyer} §8[§bLelang§8]§b\n`;
        body += `  §8  ${h.item} §8-- §e${fmt(h.price)} Koin\n`;
      } else if (h.type === "expired") {
        body += `  §8[§8Expired§8]§8 §7${h.seller}\n`;
        body += `  §8  ${h.item}\n`;
      }
      body += `  §8  ${timeAgo(h.ts)}\n\n`;
    }
...
async function aboutRules(player) {
  const feePct = getFee();
  const exFee = calcFee(1000, player);
  ...
  body += `  §eFEE PENJUALAN\n`;
  body += `${CFG.HR_THIN}\n`;
  body += `  §8├ §fFee: §e${feePct}%% §8dari harga jual\n`;
  body += `  §8├ §fDipotong §eotomatis §8saat item laku\n`;
  body += `  §8├ §a✦ §fPasang listing §aGRATIS\n`;
  body += `  §8└ §fContoh: 1.000⛃ → fee §c${fmt(exFee)}⛃ §8→ terima §a${fmt(1000 - exFee)}⛃\n\n`;
  body += `  §eLIMIT\n`;
  body += `${CFG.HR_THIN}\n`;
  ...
  try {
    ...
    for (const l of expired) {
      ...
      if (l.offerId && l.offerAmount > 0) {
        ...
        if (bidder) { addCoin(bidder, l.offerAmount); bidder.sendMessage(`§8[§eAuction§8]§e Listing expired. §e${fmt(l.offerAmount)} Koin §fdikembalikan.`); }
        else {
          pushNotif(l.offerId, `§8[§eAuction§8]§e Listing expired. §f${fmt(l.offerAmount)} Koin §edikembalikan.`);
          addPendingCoin(l.offerId, l.offerAmount);
        }
      }

      // Return item to seller — returnItemToOwner TIDAK strip passport
      const seller = world.getPlayers().find(p => p.id === l.sellerId);
      if (seller) {
        const gave = returnItemToOwner(seller, l.id, l.itemData);
        if (!gave) { addPendingItem(seller.id, l.itemData); removeCachedClone(l.id); }
        seller.sendMessage(`§8[§eAuction§8]§e §f${displayName(l.itemData)} §eexpired. Item dikembalikan.${!gave ? '\n§c⚠ Inventory penuh, item masuk pending.' : ''}`);
      } else {
        addPendingItem(l.sellerId, l.itemData);
        removeCachedClone(l.id);
    ...
    for (const l of settled) {
      ...
      if (winner) {
        const gave = giveItemFromListing(winner, l.id, l.itemData);
        if (!gave) { addPendingItem(winner.id, l.itemData); removeCachedClone(l.id); }
        playSfx(winner, SFX.BUY);
        winner.sendMessage(`§8[§aAuction§8]§a Menang! §f${itemName}\n§8  Harga : §e${fmt(l.currentBid)} Koin${!gave ? '\n§c⚠ Inventory penuh, item masuk pending.' : ''}`);
      } else {
        addPendingItem(l.bidderId, l.itemData);
        removeCachedClone(l.id);
        pushNotif(l.bidderId, `§8[§aAuction§8]§a Menang! §f${itemName} §a— §e${fmt(l.currentBid)} Koin§a.`);
      }

      // Give koin to seller (minus fee)
      const sellFee = calcFee(l.currentBid, l.sellerName);
      const sellerPayout = l.currentBid - sellFee;
      const seller = world.getPlayers().find(p => p.id === l.sellerId);
      if (sellFee > 0) trackFlow("auction_fee", -sellFee);
      if (seller) {
        // [§2] Fall back to pending if scoreboard write fails — never destroy coin.
        if (addCoin(seller, sellerPayout)) {
          playSfx(seller, SFX.SOLD);
          seller.sendMessage(`§8[§aAuction§8]§a Terjual! §f${itemName}\n§8  Pembeli : §f${l.bidderName}\n§8  Harga   : §e${fmt(l.currentBid)} Koin\n§8  Fee     : §c-${fmt(sellFee)} Koin\n§8  Terima  : §a${fmt(sellerPayout)} Koin`);
        } else {
          addPendingCoin(l.sellerId, sellerPayout);
          console.warn(`[Auction] settle payout fallback to pending: ${l.sellerName} ${sellerPayout}`);
          seller.sendMessage(`§8[§aAuction§8]§a Terjual! §f${itemName} §e${fmt(sellerPayout)} Koin §7masuk pending — klaim ulang dengan buka menu auction.`);
        }
      } else {
        addPendingCoin(l.sellerId, sellerPayout);
    ...
    // Cleanup clone cache — hapus entries tanpa listing aktif
    const activeIds = getActiveListings().map(l => l.id);
    cleanupCloneCache(activeIds);
      ...
      (origin) => {
        ...
        system.run(async () => {
          try {
            ...
            if (isFeatureOnNPC("auction")) {
              player.sendMessage("§8[Sistem]§c Fitur ini tersedia lewat NPC.\n§7Kunjungi NPC di spawn untuk mengakses Auction.");
              return;
            }
          } catch {}
          // Fallback: buka langsung
          if (isPurgeActive()) { player.sendMessage("§8[Sistem] §cPilar Energi offline — auction tidak tersedia."); return; }
          if (activeSessions.has(player.id)) return;
          openAuction(player).catch(e => { if (!e?.isUIClose) console.error("[Auction] error:", e); });
        });
        return { status: 0 };
      }
    );
...
world.afterEvents.playerSpawn.subscribe(({ player, initialSpawn }) => {
  ...
  }, 240); // was 120 — stagger last in join chain
});
