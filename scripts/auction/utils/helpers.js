...
export function setCoin(player, n) {
  try {
    ensureCoinObj()?.setScore(player.scoreboardIdentity ?? player, Math.max(0, Math.floor(n)));
    return true;
  } catch (e) {
    console.error("[Auction] setCoin:", e);
    return false;
  }
}
// addCoin returns success boolean — purchase paths MUST check, refund paths
// can ignore (silent fail there falls back to pending).
...
export function onPlayerLeave(playerId) {
  ...
  _sellCdMap.delete(playerId);
}

const _sellCdMap = new Map();
export function checkSellCooldown(p) {
  const last = _sellCdMap.get(p.id) ?? 0;
  const diff = Date.now() - last;
  if (diff < CFG.SELL_COOLDOWN_MS) return Math.ceil((CFG.SELL_COOLDOWN_MS - diff) / 1000);
  return 0;
}
export function setSellCooldown(p) { _sellCdMap.set(p.id, Date.now()); }

const FIRST_SALE_OBJ = "auc_first";
function ensureFirstSaleObj() {
  return world.scoreboard.getObjective(FIRST_SALE_OBJ)
    ?? world.scoreboard.addObjective(FIRST_SALE_OBJ, "First Sale");
}
export function hasFirstSaleBonus(p) {
  try { return (ensureFirstSaleObj()?.getScore(p.scoreboardIdentity ?? p) ?? 0) > 0; }
  catch { return false; }
}
export function markFirstSale(p) {
  try { ensureFirstSaleObj()?.setScore(p.scoreboardIdentity ?? p, 1); } catch {}
}
