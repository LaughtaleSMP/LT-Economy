...
// Menyimpan ItemStack.clone() untuk listing aktif.
// Ini penting untuk item yang punya data internal yang tidak bisa
// di-serialize ke JSON (banner patterns, potion effects, dll).
// Cache hilang saat server restart — fallback ke deserialize.
//
// MEMORY SAFETY:
// - Max 50 entries (CFG.MAX_GLOBAL biasanya <= 50)
// - Auto-cleanup saat listing selesai
// - Entries hanya menyimpan 1 ItemStack clone per listing
// ═══════════════════════════════════════════════════════════
const _cloneCache = new Map(); // listingId → ItemStack clone
const _CACHE_MAX = 60; // sedikit lebih dari MAX_GLOBAL untuk safety margin

/**
 * Simpan clone item ke cache.
 * Dipanggil saat listing dibuat.
 */
export function cacheItemClone(listingId, itemStack) {
  try {
    // Memory guard: jika cache penuh, hapus entry terlama
    if (_cloneCache.size >= _CACHE_MAX) {
      const oldest = _cloneCache.keys().next().value;
      _cloneCache.delete(oldest);
      console.log("[Auction] cacheItemClone: cache penuh, hapus entry terlama:", oldest);
    }
    _cloneCache.set(listingId, itemStack.clone());
  } catch (e) {
    console.error("[Auction] cacheItemClone gagal:", e);
  }
}

/**
 * Ambil clone item dari cache (tanpa menghapus).
 * Return null jika tidak ada (server restart).
 */
export function getCachedClone(listingId) {
  return _cloneCache.get(listingId) ?? null;
}

/**
 * Hapus clone dari cache.
 * Dipanggil saat listing SELESAI (sold/expired/cancelled) dan item sudah di-restore.
 */
export function removeCachedClone(listingId) {
  _cloneCache.delete(listingId);
}

/**
 * Bersihkan cache entries yang tidak punya listing aktif lagi.
 * Dipanggil periodik dari prune interval untuk mencegah memory leak.
 * @param {string[]} activeIds - array listing ID yang masih aktif
 */
export function cleanupCloneCache(activeIds) {
  const activeSet = new Set(activeIds);
  let cleaned = 0;
  for (const id of _cloneCache.keys()) {
    if (!activeSet.has(id)) {
      _cloneCache.delete(id);
      cleaned++;
    }
  }
  if (cleaned > 0) console.log(`[Auction] cleanupCloneCache: ${cleaned} stale entries dihapus. Cache: ${_cloneCache.size}`);
}
...
/**
 * Buat item untuk dikembalikan ke player.
 * Prioritas: clone cache → deserialize fallback.
 * CATATAN: TIDAK menghapus cache — cache dihapus oleh caller setelah berhasil.
 */
export function restoreItem(listingId, itemData) {
  // Coba dari clone cache dulu (preservasi banner patterns, dll)
  const cached = getCachedClone(listingId);
  if (cached) {
    try {
      const restored = cached.clone();
      restored.amount = itemData.amount ?? 1;
      return restored;
    } catch (e) {
      console.warn("[Auction] restoreItem clone gagal, fallback ke deserialize:", e);
    }
  }
  // Fallback ke deserialize (setelah server restart)
...
/**
 * Deteksi apakah item punya data kompleks yang tidak bisa di-serialize sempurna.
 * Jika server restart saat item ini di-listing, data custom bisa hilang.
 */
export function hasComplexData(itemStack) {
  const id = itemStack.typeId.replace("minecraft:", "");
  // Shulker box dengan isi container
  if (id.endsWith("_shulker_box") || id === "shulker_box") {
    try {
      const inv = itemStack.getComponent("minecraft:inventory");
      if (inv && inv.container) {
        for (let i = 0; i < inv.container.size; i++) {
          if (inv.container.getItem(i)) return true;
        }
      }
    } catch {}
  }
  // Banner dengan patterns
  if (id.endsWith("_banner") || id === "banner") {
    if (!itemStack.isStackable || itemStack.nameTag) return true;
  }
  // Decorated pots
  if (id === "decorated_pot") return true;
  // Written books punya content yang complex
  if (id === "written_book") return true;
  // Suspicious stew punya effects
  if (id === "suspicious_stew") return true;
  return false;
}
// ═══════════════════════════════════════════════════════════
// SERIALIZE — ubah ItemStack jadi plain object
// Menyimpan SEMUA properties agar item bisa di-restore 100% identik
// ═══════════════════════════════════════════════════════════
export function serializeItem(itemStack) {
  ...
  // Dyeable (leather armor, banner base color, etc.)
  try {
    const dye = itemStack.getComponent("minecraft:dyeable");
    if (dye && dye.color) {
      data.dyeColor = dye.color;
    }
  } catch {}

  // canDestroy / canPlaceOn (adventure mode)
  try {
    const cd = itemStack.getCanDestroy();
    if (cd && cd.length > 0) data.canDestroy = cd;
  } catch {}
  try {
    const cp = itemStack.getCanPlaceOn();
    if (cp && cp.length > 0) data.canPlaceOn = cp;
  } catch {}

  // Dynamic Properties (item-level custom data)
  try {
    const dpIds = itemStack.getDynamicPropertyIds();
    if (dpIds && dpIds.length > 0) {
      data.dynamicProps = {};
      for (const id of dpIds) {
        data.dynamicProps[id] = itemStack.getDynamicProperty(id);
      }
    }
  } catch {}

  // Container (shulker box, dll) — serialize isi inventory
  try {
    const invComp = itemStack.getComponent("minecraft:inventory");
    if (invComp && invComp.container) {
      const cont = invComp.container;
      const items = [];
      let hasItems = false;
      for (let i = 0; i < cont.size; i++) {
        const slotItem = cont.getItem(i);
        if (slotItem) {
          // Recursive serialize — shulker dalam shulker tidak umum tapi safe
          items.push({ slot: i, data: serializeItem(slotItem) });
          hasItems = true;
        }
      }
      if (hasItems) data.containerItems = items;
    }
  } catch {}
...
// Restore semua properties yang telah di-serialize
// ═══════════════════════════════════════════════════════════
  ...
  try {
    ...
    if (data.dyeColor) {
      try {
        ...
        if (dye) dye.color = data.dyeColor;
      } catch {}
    }

    // canDestroy / canPlaceOn
    if (data.canDestroy && data.canDestroy.length > 0) {
      try { item.setCanDestroy(data.canDestroy); } catch {}
    }
    if (data.canPlaceOn && data.canPlaceOn.length > 0) {
      try { item.setCanPlaceOn(data.canPlaceOn); } catch {}
    }

    // Dynamic Properties
    if (data.dynamicProps) {
      for (const [dpId, dpVal] of Object.entries(data.dynamicProps)) {
        try { item.setDynamicProperty(dpId, dpVal); } catch {}
      }
    }

    // Container (shulker box, dll) — restore isi inventory
    if (data.containerItems && data.containerItems.length > 0) {
      try {
        const invComp = item.getComponent("minecraft:inventory");
        if (invComp && invComp.container) {
          const cont = invComp.container;
          for (const entry of data.containerItems) {
            try {
              const innerItem = deserializeItem(entry.data);
              if (innerItem) cont.setItem(entry.slot, innerItem);
            } catch {}
          }
        }
      } catch (ce) {
        console.warn("[Auction] deserializeItem container restore gagal:", ce);
      }
    }
...
/**
 * Internal: tempatkan ItemStack ke slot kosong di inventory player.
 */
function _placeItem(player, item, debugTypeId) {
  try {
    const inv = player.getComponent("minecraft:inventory")?.container;
    if (!inv) {
      console.error("[Auction] _placeItem: tidak bisa akses inventory container");
      return false;
    }
    ...
    for (let i = 0; i < inv.size; i++) {
      if (!inv.getItem(i)) {
        inv.setItem(i, item);
        // Verifikasi item benar-benar masuk
        const check = inv.getItem(i);
        if (check && check.typeId === item.typeId) {
          return true;
        }
        // Jika verifikasi gagal, coba slot lain
        console.log("[Auction] _placeItem: setItem slot", i, "gagal verifikasi, coba slot lain");
      }
    }

    console.warn("[Auction] _placeItem: inventory penuh untuk", player.name, "item:", debugTypeId);
    return false;
  } catch (e) {
    console.error("[Auction] _placeItem gagal:", e, "item:", debugTypeId);
    return false;
  }
}
 ...
 * 
 * PENTING: Menggunakan ItemStack.clone() untuk fallback agar
 * banner patterns, dye colors, dan internal NBT data terpreservasi 100%.
 * 
...
export function takePartialFromSlot(player, slot, quantity) {
  try {
    ...
    // Simpan jumlah original SEBELUM modifikasi apapun
    const originalAmount = item.amount;
    const remainAmount = originalAmount - quantity;

    // Clone item SEBELUM modifikasi apapun — ini mempreservasi 100% data
    // termasuk banner patterns, NBT data, dll yang tidak bisa di-serialize
    const originalClone = item.clone();

    // Serialize untuk return value (data yang disimpan di listing)
    const data = serializeItem(item);
    data.amount = quantity;

    if (quantity === originalAmount) {
      // Ambil semua — hapus slot
      inv.setItem(slot, undefined);
    } else {
      // Ambil sebagian — set amount langsung di ItemStack asli
      // Ini mempertahankan semua components/properties termasuk banner patterns
      try {
        item.amount = remainAmount;
        inv.setItem(slot, item);
      } catch (e2) {
        // Fallback: gunakan clone() untuk membuat remaining stack
        // clone() mempreservasi SEMUA data termasuk banner patterns
        console.warn("[Auction] Direct amount set failed, using clone fallback:", e2);
        try {
          const remaining = originalClone.clone();
          remaining.amount = remainAmount;
          inv.setItem(slot, remaining);
        } catch (e3) {
          // Gagal total — restore slot ke kondisi original
          console.error("[Auction] takePartialFromSlot: clone fallback gagal! Restoring original.", e3);
          try { originalClone.amount = originalAmount; inv.setItem(slot, originalClone); } catch {}
          return null;
        }
      }

      // Verifikasi bahwa slot benar-benar terupdate
      const verify = inv.getItem(slot);
      if (!verify) {
        // Slot kosong padahal seharusnya masih ada sisa — restore ke original
        console.error("[Auction] takePartialFromSlot: slot kosong setelah partial take! Restoring original.");
        try { originalClone.amount = originalAmount; inv.setItem(slot, originalClone); } catch {}
        return null;
      }
      if (verify.amount !== remainAmount) {
        console.warn("[Auction] takePartialFromSlot: amount mismatch! expected:", remainAmount, "got:", verify.amount);
      }
    }
