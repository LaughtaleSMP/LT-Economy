...
import { syncLeaderboard, pollTopupQueue, pollRecoveryQueue, pollMimiCommands, microSyncPositions } from "./sync.js";
...
world.afterEvents.playerSpawn.subscribe(({ player, initialSpawn }) => {
  ...
  }, 140); // was 60 — stagger after daily(80) to spread DP load
});
  ...
  customCommandRegistry.registerCommand({
    ...
    system.run(async () => {
      try {
        ...
        if (isFeatureOnNPC("leaderboard")) {
          player.sendMessage("§8[Sistem]§c Fitur ini tersedia lewat NPC.\n§7Kunjungi NPC di spawn untuk mengakses Leaderboard.");
          return;
        }
      } catch {}
      showLeaderboard(player).catch(e => { if (!e?.isUIClose) console.warn("[LB] UI:", e); });
    });
    return { status: 0 };
  });
...
system.runInterval(() => {
  pollTopupQueue().catch(_filterCircuit("[Topup-Poll]"));
  pollRecoveryQueue().catch(_filterCircuit("[Recovery-Poll]"));
  pollMimiCommands().catch(_filterCircuit("[MimiCmd-Poll]"));
}, 600);
