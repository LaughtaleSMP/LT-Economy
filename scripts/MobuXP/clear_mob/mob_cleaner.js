...
const MOB_GROUPS = {
  ...
  Hostile: ['minecraft:creeper','minecraft:spider','minecraft:cave_spider','minecraft:enderman','minecraft:endermite','minecraft:silverfish','minecraft:witch','minecraft:slime','minecraft:vex','minecraft:breeze'],
  // [FIX] minecraft:shulker dihapus dari Hostile — shulker penting di The End, jangan ikut dibersihkan
...
let _hasTargets = false  // cached — recomputed on config change only
...
function saveConfig() {
  ...
  _hasTargets = MOB_IDS.some(id => cfg.mobs[id])
}
...
// -- Core --
// [PERF v4.0] Uses targeted per-type getEntities() queries instead of
// unfiltered dim.getEntities(). Engine-level type filter is significantly
// faster than fetching all entities then filtering client-side.
// Player position cached once per cleanMobs() call.
...
function cleanMobs() {
  ...
  const targets = new Set(MOB_IDS.filter(id => cfg.mobs[id]))
  ...
  for (const dimId of DIMENSIONS) {
    try {
      ...
      // [PERF v4.0] Per-type queries — engine filters at native level
      // Much faster than fetching all entities when only targeting specific types
      for (const typeId of targets) {
        try {
          for (const e of dimension.getEntities({ type: typeId })) {
            try {
              if (!isValid(e) || (e.nameTag && e.nameTag.trim())) continue
...
system.runInterval(() => {
  ...
  if (shouldSkipHeavy()) return  // [PERF] Skip when TPS stressed — auto_throttle handles cleanup

  tick += 20
  const remaining  = cfg.interval - tick

  if (_hasTargets && !warned1min && cfg.interval > WARN_1MIN_TICKS && remaining <= WARN_1MIN_TICKS && remaining > 0) {
    warned1min = true
    broadcast('§8[§6Mob Cleaner§8]§6 §eMonster tanpa nametag akan dibersihkan dalam §f1 menit§e!')
    playSoundAll('note.pling', 1, 0.5)
  }

  const warn10pctAt = Math.floor(cfg.interval * WARN_THRESHOLD)
  if (_hasTargets && !warned && remaining <= warn10pctAt && Math.abs(warn10pctAt - WARN_1MIN_TICKS) > 100) {
    warned = true
    broadcast('§8[§6Mob Cleaner§8]§6 §eMonster dibersihkan dalam §f' + formatSecs(remaining))
    playSoundAll('note.pling', 1, 1.2)
  }

  if (_hasTargets && remaining <= ACTIONBAR_SHOW_TICKS) {
    const bar = '§6Mob Cleaner ' + buildBar(remaining) + ' §e' + formatSecs(remaining)
    setBarAll(bar, 10, 40)

    // [PERF] Skip sound effects when TPS is stressed
    if (!shouldSkipHeavy()) {
      for (const p of world.getPlayers()) {
        try {
  ...
  if (tick >= cfg.interval) {
    ...
    broadcast('§8[§6Mob Cleaner§8]§6 §eDibersihkan §f' + count + ' §emob.')
...
async function openGroupConfig(player, groupName) {
  ...
  const form = new ModalFormData().title('§8 ♦ §6' + groupName.toUpperCase() + '§r §8♦ §r')
...
async function openMobMenu(player) {
  const form = new ActionFormData().title('§8 ♦ §6PILIH MOB§r §8♦ §r')
...
async function openIntervalConfig(player) {
  const form = new ModalFormData().title('§8 ♦ §6INTERVAL§r §8♦ §r')
