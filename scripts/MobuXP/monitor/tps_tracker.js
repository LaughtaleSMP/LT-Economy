...
/**
 * Detect TPS trend from history — computed on-demand, zero ongoing cost.
 * Compares average of last 3 samples vs previous 3 samples.
 */
export function getTPSTrend() {
  if (_history.length < 6) return { trend: "stable", label: "─ Stabil", delta: 0 };
  const recent = _history.slice(-3);
  const older  = _history.slice(-6, -3);
  const avgR = recent.reduce((a, b) => a + b, 0) / recent.length;
  const avgO = older.reduce((a, b) => a + b, 0) / older.length;
  const delta = Math.round((avgR - avgO) * 10) / 10;
  if (delta < -1.5) return { trend: "down", label: "▼ Turun", delta };
  if (delta > 1.5)  return { trend: "up",   label: "▲ Naik",  delta };
  return { trend: "stable", label: "─ Stabil", delta };
}
