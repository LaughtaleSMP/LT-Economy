...
//
// ITEM CLEANUP POLICY:
//   - DISABLED — items are NEVER auto-cleaned
//   - Manual clean available via admin UI (manualCleanItems)
//
// [PERF v4.0]:
...
const PROTECTED = new Set([
  ...
  "minecraft:npc", "lt:market_npc", "minecraft:player",
...
// ═══════════════════════════════════════════════════════════
// [PERF] Stagger +15 ticks — avoid collision with entity_counter(+30)/pigman(+50)
...
}, CAP_CHECK_TICKS); }, 15); // stagger offset
...
const CHUNK_DENSITY_TRIGGER = 0;  // Always scan — even 50 silverfish in 1 chunk causes lag

// Per-family overrides: some mobs need tighter caps
const FAMILY_CHUNK_CAP = {
  monster:  20,  // default hostile
  animal:   20,  // passive
  villager: 20,  // max 20 per chunk — iron golem farm needs 20
};
// Specific mob type hard caps (swarm mobs)
const TYPE_CHUNK_CAP = {
  "minecraft:silverfish": 8,
  "minecraft:endermite":  8,
};

// Named entity HARD CAP — total (unnamed+named) per chunk per family.
// Unnamed are removed first, named only if still over this higher cap.
// This prevents infinite named mob farms (e.g., 100 named villagers).
const NAMED_HARD_CAP = {
  monster:  20,  // max 20 per chunk — same rule for all
  animal:   20,  // max 20 per chunk — same rule for all
  villager: 20,  // same as unnamed — simple rule: max 20, period
};
...
const DENSITY_PROTECTED = new Set([
  ...
  "minecraft:elder_guardian", "minecraft:shulker", "minecraft:npc", "lt:market_npc",
]);
...
// [PERF] Stagger +70 ticks — heaviest query, isolated from all other intervals
system.runTimeout(() => { system.runInterval(() => {
  if (!_enabled) return;
  ...
  try {
    ...
    if (dim.getPlayers().length === 0) return;
    ...
    for (const family of DENSITY_FAMILIES) {
      ...
      const familyCap = FAMILY_CHUNK_CAP[family] ?? CHUNK_DENSITY_CAP;
      if (countKey && (ec[countKey] || 0) < familyCap) continue;
      try {
        const entities = dim.getEntities({ families: [family] });

        // Pass 1: Type-specific hard caps (silverfish, endermite)
        const typeGroups = new Map();
        for (const e of entities) {
          const tid = e.typeId;
          if (!tid || DENSITY_PROTECTED.has(tid)) continue;
          if (e.nameTag?.trim()) continue;
          const typeCap = TYPE_CHUNK_CAP[tid];
          if (typeCap !== undefined) {
            const cx = Math.floor(e.location.x) >> 4;
            const cz = Math.floor(e.location.z) >> 4;
            const key = tid + ":" + cx + "," + cz;
            let arr = typeGroups.get(key);
            if (!arr) { arr = []; typeGroups.set(key, arr); }
            arr.push(e);
          }
        }
        // Cull type-specific excess
        for (const [key, arr] of typeGroups) {
          const tid = key.split(":")[0];
          const cap = TYPE_CHUNK_CAP[tid];
          if (arr.length > cap) {
            for (let i = cap; i < arr.length; i++) {
              try { arr[i].remove(); removed.add(arr[i]); chunkRemoved++; } catch {}
            }
          }
        }

        // Pass 2: Standard density cap (per family)
        const { chunkMap, regionMap } = _buildDensityMaps(entities);
        chunkRemoved  += _cullExcess(chunkMap,  familyCap,          removed);
        regionRemoved += _cullExcess(regionMap, REGION_DENSITY_CAP, removed);

        // Pass 3: Named entity hard cap — includes named mobs
        // Monster: per FAMILY (20 total hostile per chunk, all types combined)
        // Animal/Villager: per TYPE (20 cow + 20 pig = OK)
        const hardCap = NAMED_HARD_CAP[family] ?? 20;
        const perFamily = (family === "monster"); // monster = pool all types together
        const namedChunkMap = new Map();
        for (const e of entities) {
          try {
            ...
            const cx = Math.floor(e.location.x) >> 4;
            const cz = Math.floor(e.location.z) >> 4;
            // Monster: group by chunk only. Animal/Villager: group by chunk+type
            const ck = perFamily ? (cx + "," + cz) : (tid + ":" + cx + "," + cz);
            let arr = namedChunkMap.get(ck);
            if (!arr) { arr = []; namedChunkMap.set(ck, arr); }
            arr.push(e);
          } catch { }
        }
        for (const [, arr] of namedChunkMap) {
          if (arr.length <= hardCap) continue;
          // Sort: unnamed first (remove those), named last (protect)
          arr.sort((a, b) => {
            const aV = a.isValid ? 1 : 0;
            const bV = b.isValid ? 1 : 0;
            if (aV !== bV) return aV - bV;
            const aN = (aV && a.nameTag?.trim()) ? 1 : 0;
            const bN = (bV && b.nameTag?.trim()) ? 1 : 0;
            return aN - bN;
          });
          for (let i = hardCap; i < arr.length; i++) {
            try {
              if (arr[i].isValid) { arr[i].remove(); removed.add(arr[i]); chunkRemoved++; }
            } catch { }
          }
        }
      } catch { }
