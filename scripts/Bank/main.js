...
function setCoin(player, n) {
  try {
    ...
    console.error("[Bank] setCoin error:", e);
    return false;
  }
}

// addCoin returns success boolean. Callers MUST check on transactional paths
// (see executeTransfer rollback) — silent fail = coin destruction (§2 iron rule).
...
// [PhD-v2] Migration: old default 5% → new default 2% (flat).
// Kalau stored value == old default, migrate otomatis (admin tidak kustomisasi).
// Kalau admin memang set value custom, respect pilihan mereka.
const _OLD_DEFAULT_TAX = 5;
let _settingsCache = null;
export const getSettings  = ()  => {
  if (!_settingsCache) {
    const stored = dp.get(CFG.K_SETTINGS, null);
    if (stored === null) {
      // Never set — use new default
      _settingsCache = { taxPct: CFG.TAX_PERCENT, _v: 2 };
    } else if ((stored._v ?? 1) < 2 && stored.taxPct === _OLD_DEFAULT_TAX) {
      // Legacy: stored old default → migrate silently to new default
      _settingsCache = { taxPct: CFG.TAX_PERCENT, _v: 2 };
      dp.set(CFG.K_SETTINGS, _settingsCache);
      console.log(`[Bank] Migrated tax ${_OLD_DEFAULT_TAX}%→${CFG.TAX_PERCENT}% (PhD velocity reform).`);
    } else {
      _settingsCache = { _v: 2, ...stored };
    }
  }
...
export const getTax = () => Math.max(0, getSettings().taxPct + _getPolicyAdj());
export function calcTax(n) {
  const base = getTax();
  // [DYNAMIC] Reads tiered extra from CFG.TAX_BRACKETS
  let extra = 0;
  for (const b of CFG.TAX_BRACKETS) {
...
async function executeTransfer(from, to, amount, note = "") {
  return withLock(from.id, () => {
    ...
    // [§2 iron rule] Atomic 2-phase: deduct sender, then credit receiver.
    // If credit fails, rollback sender to prevent coin destruction.
    if (!setCoin(from, curBal - totalOut)) {
      console.warn(`[Bank] transfer aborted: setCoin(from) failed ${from.name}→${to.name} ${amount}`);
      return { ok: false, err: "internal" };
    }
    if (!addCoin(to, amount)) {
      // Rollback sender — restore previous balance
      const restored = setCoin(from, curBal);
      console.error(
        `[Bank] CREDIT FAILED — rollback ${restored ? "ok" : "FAILED"}. ` +
        `${from.name}→${to.name} ${amount} koin. Manual review needed.`
      );
      return { ok: false, err: "internal" };
    }
    ...
    if (tax > 0) trackFlow("bank_tax", -tax);

    // [PhD-v2] Demurrage activity reset — both sender & receiver
    try { pointActivity(from); } catch {}
    try { pointActivity(to); } catch {}
...
// OPEN BANK
// ═══════════════════════════════════════════════════════════
export async function openBankMenu(player) {
...
async function uiTransfer(player) {
  ...
  if (!Number.isFinite(amount) || amount < CFG.MIN_TRANSFER) {
    player.sendMessage(`§8[§cBank§8]§c Jumlah tidak valid. Minimal §f${fmt(CFG.MIN_TRANSFER)} Koin.`); return;
  }
  if (amount > CFG.MAX_TRANSFER) {
    player.sendMessage(`§8[§cBank§8]§c Melebihi batas per transaksi (§f${fmt(CFG.MAX_TRANSFER)} Koin§c).`); return;
  }
  if (amount > remain) {
    player.sendMessage(`§8[§cBank§8]§c Sisa limit hari ini: §f${fmt(remain)} Koin.`); return;
  }
  ...
  if (myCoin < total) {
    player.sendMessage(`§8[§cBank§8]§c Saldo tidak cukup! Butuh §f${fmt(total)}` + (taxAmt > 0 ? ` §c(${fmt(amount)} + pajak ${fmt(taxAmt)}).` : `.`));
    return;
  }
  ...
  if (!tgt) {
    player.sendMessage(`§8[§cBank§8]§c ${target.name} sudah offline, transfer dibatalkan.`);
    return;
  }
  ...
  if (result === false) {
    player.sendMessage("§8[§cBank§8]§c Ada transaksi lain berjalan. Coba lagi sebentar.");
    return;
  }
  if (!result.ok) {
    player.sendMessage(result.err === "daily_limit"
      ? "§8[§cBank§8]§c Limit transfer harian terlampaui!"
      : "§8[§cBank§8]§c Saldo tidak cukup!"
    );
    return;
  }
  ...
  tgt.sendMessage(
    `§8[§aBank§8]§a Kamu menerima koin!\n` +
...
async function uiSendRequest(player) {
  ...
  if ((inboxCounts.get(target.id) ?? 0) >= CFG.MAX_PENDING_REQ) {
    player.sendMessage(`§8[§cBank§8]§c Inbox §f${target.name} §cpenuh.`);
    return;
  }
  ...
  if (!Number.isFinite(amount) || amount < CFG.MIN_TRANSFER) {
    player.sendMessage(`§8[§cBank§8]§c Jumlah minimal §f${fmt(CFG.MIN_TRANSFER)} Koin.`); return;
  }
  if (!reason) {
    player.sendMessage("§8[§cBank§8]§c Alasan tidak boleh kosong!"); return;
  }

  const tgt = world.getPlayers().find(p => p.id === target.id);
  if (!tgt) { player.sendMessage(`§8[§cBank§8]§c ${target.name} sudah offline.`); return; }

  const req = { id: genId(), fromId: player.id, fromName: player.name, amount, reason, createdAt: Date.now() };
  if (!addIncomingReq(tgt.id, req)) {
    player.sendMessage(`§8[§cBank§8]§c Inbox ${tgt.name} penuh.`); return;
  }

  playSfx(tgt, SFX.REQUEST);
  player.sendMessage(
    `§8[§aBank§8]§a Request terkirim!\n` +
    `§7 Kepada : §f${tgt.name}\n` +
    `§7 Jumlah : §e${fmt(amount)} Koin\n` +
    `§7 Alasan : §f${reason}`
  );
  tgt.sendMessage(
    `§8[§eBank§8]§e Permintaan koin masuk!\n` +
    `§7 Dari   : §f${player.name}\n` +
    `§7 Jumlah : §e${fmt(amount)} Koin\n` +
    `§7 Alasan : §f${reason}\n` +
...
async function uiInbox(player) {
  while (true) {
    ...
    if (detail.selection === 0) {
      ...
      player.sendMessage(`§8[§7Bank§8]§7 Request dari §f${req.fromName} §7ditolak.`);
      const sender = world.getPlayers().find(p => p.id === req.fromId);
      if (sender) {
        playSfx(sender, SFX.DECLINE);
        sender.sendMessage(`§8[§cBank§8]§c Request koin ke §f${player.name} §cditolak.`);
      } else {
        pushPendingNotif(req.fromId, `§8[§cBank§8]§c Request koin ke §f${player.name} §cditolak.`);
      }
      continue;
    }

    if (getCoin(player) < total) {
      player.sendMessage(`§8[§cBank§8]§c Saldo tidak cukup. Butuh §f${fmt(total)} Koin.`);
      continue;
    }
    ...
    if (!requester) {
      ...
      pushPendingNotif(req.fromId,
        `§8[§eBank§8]§e §f${player.name} §emau menerima requestmu tapi kamu offline saat itu.\n§8 Kirim request lagi saat kamu online.`
      );
      player.sendMessage(`§8[§cBank§8]§c §f${req.fromName} §csudah offline. Request dihapus.`);
      continue;
    }
    ...
    if (!result.ok) {
      // Keep request — player bisa coba lagi setelah top-up
      player.sendMessage(result.err === "daily_limit"
        ? "§8[§cBank§8]§c Limit transfer harian terlampaui!"
        : result.err === "internal"
          ? "§8[§cBank§8]§c Transfer gagal, coba lagi sebentar."
          : "§8[§cBank§8]§c Saldo tidak cukup!"
      );
      continue;
    }

    // Only remove request after successful transfer.
    removeReq(player.id, req.id);
...
async function uiAdmin(player) {
  if (!player.hasTag(CFG.ADMIN_TAG)) { player.sendMessage("§8[§cBank§8]§c Akses ditolak."); return; }
...
async function adminGiveCoin(admin) {
  ...
  if (!Number.isFinite(amount) || amount <= 0) { admin.sendMessage("§8[§cBank§8]§c Angka tidak valid."); return; }

  const tgt = world.getPlayers().find(p => p.id === target.id);
  if (!tgt) { admin.sendMessage("§8[§cBank§8]§c Player sudah offline."); return; }

  addCoin(tgt, amount);
  playSfx(admin, SFX.ADMIN);
  playSfx(tgt,   SFX.RECEIVE);
  admin.sendMessage(`§8[§aBank Admin§8]§a Diberikan §f${fmt(amount)} §aKoin ke §f${tgt.name}. Saldo: §e${fmt(getCoin(tgt))}`);
  tgt.sendMessage(`§8[§aBank§8]§a Admin memberimu §e${fmt(amount)} Koin§a! Saldo: §e${fmt(getCoin(tgt))}`);
}
...
async function adminDeductCoin(admin) {
  ...
  if (deducted === false) { admin.sendMessage("§8[§cBank§8]§c Player sedang transaksi. Coba lagi."); return; }

  playSfx(admin, SFX.ADMIN);
  admin.sendMessage(`§8[§aBank Admin§8]§a Dikurangi §f${fmt(deducted)} §aKoin dari §f${tgt.name}. Saldo: §e${fmt(getCoin(tgt))}`);
  tgt.sendMessage(`§8[§cBank§8]§c Admin mengurangi §c${fmt(deducted)} Koin§c. Saldo: §e${fmt(getCoin(tgt))}`);
}
...
async function adminSetTax(admin) {
  ...
  if (!Number.isFinite(newTax) || newTax < 0 || newTax > 50) {
    admin.sendMessage("§8[§cBank§8]§c Nilai pajak tidak valid."); return;
  }

  settings.taxPct = newTax;
  saveSettings(settings);
  playSfx(admin, SFX.ADMIN);
  admin.sendMessage(`§8[§aBank Admin§8]§a Pajak diubah ke §f${newTax}%`);
  world.sendMessage(`§8[§eBank§8]§e Pajak transfer diubah menjadi §f${newTax}%.`);
}
...
async function adminResetDaily(admin) {
  ...
  playSfx(admin, SFX.ADMIN);
  admin.sendMessage(`§8[§aBank Admin§8]§a Limit harian §f${target.name} §adireset.`);
  const tgt = world.getPlayers().find(p => p.id === target.id);
  if (tgt) tgt.sendMessage("§8[§aBank§8]§a Limit transfer harianmu direset oleh admin.");
}
...
async function adminClearHistory(admin) {
  ...
  playSfx(admin, SFX.ADMIN);
  admin.sendMessage(`§8[§aBank Admin§8]§a Riwayat §f${target.name} §adihapus.`);
}
...
async function pickOnlinePlayer(admin, title) {
  ...
  if (!players.length) { admin.sendMessage("§8[§cBank§8]§c Tidak ada player lain online."); return null; }
  ...
  try {
    init.customCommandRegistry.registerCommand(
      ...
      (origin) => {
        ...
        if (isPurgeActive()) {
          system.run(() => player.sendMessage("§8[Sistem] §cPilar Energi offline — bank tidak tersedia."));
          return;
        }
        ...
        system.run(async () => {
          try {
            ...
            if (isFeatureOnNPC("bank")) {
              player.sendMessage("§8[Sistem]§c Fitur ini tersedia lewat NPC.\n§7Kunjungi NPC di spawn untuk mengakses Bank.");
              return;
            }
          } catch {}
          openBankMenu(player).catch(e => { if (!e?.isUIClose) console.error("[Bank] error:", e); });
        });
        return { status: 0 };
      }
    );
  ...
  system.runTimeout(() => {
    try {
      ...
      if (reqs.length > 0) {
        ...
        live.sendMessage(
          `§8[§eBank§8]§e Kamu punya §f${reqs.length} §epermintaan koin masuk!\n§8 Buka /bank -> Permintaan`
        );
      }
    } catch {}
  }, 200); // was 100 — stagger between gacha(180) and auction(240)
});
