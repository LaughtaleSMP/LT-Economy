...
system.beforeEvents.startup.subscribe(({ customCommandRegistry }) => {
  customCommandRegistry.registerCommand({
    ...
    if (isPurgeActive()) {
      system.run(() => player.sendMessage("§8[Sistem] §cPilar Energi offline — daily reward tidak tersedia."));
      return;
    }
    system.run(async () => {
      try {
        const { isFeatureOnNPC } = await import("../npc/market.js");
        if (isFeatureOnNPC("daily")) {
          player.sendMessage("§8[Sistem]§c Fitur ini tersedia lewat NPC.\n§7Kunjungi NPC di spawn untuk mengakses Daily Quest.");
          return;
        }
      } catch {}
      openDailyMenu(player).catch(e => { if (!e?.isUIClose) console.warn("[Daily] UI:", e); });
    });
    return { status: 0 };
  });
});
...
world.afterEvents.playerSpawn.subscribe(({ player, initialSpawn }) => {
  ...
  // [PERF] Stagger +80 ticks — heaviest join handler, isolated from migrate(+20)
  system.runTimeout(() => {
    try {
  ...
  }, 80); // was 40 — stagger to avoid collision with migrate(10)/leaderboard(60)
});
