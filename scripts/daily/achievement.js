...
/** Peek reward without marking claimed. Returns 0 if not claimable. */
export function peekAchievement(pid, achId) {
  const data = getStatsData(pid);
...
/** Commit claim — call after coin successfully credited. */
export function commitAchievement(pid, achId) {
  const data = getStatsData(pid);
...
/** Legacy: peek + commit in one call. */
export function claimAchievement(pid, achId) {
  const reward = peekAchievement(pid, achId);
  if (reward > 0) commitAchievement(pid, achId);
  return reward;
}
