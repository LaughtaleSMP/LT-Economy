...
/** Peek reward amount without marking claimed. Returns 0 if not claimable. */
export function peekQuestReward(pid, tier, questIndex) {
  const t = TIERS[tier];
  const data = getQuestData(pid, tier);
...
/** Commit claim — mark as claimed. Call after coin successfully credited. */
export function commitQuestClaim(pid, tier, questIndex) {
  const data = getQuestData(pid, tier);
  if (questIndex < 0 || questIndex >= data.quests.length) return false;
  const q = data.quests[questIndex];
...
/** Legacy: claim + mark in one step. Kept for callers that don't fail-handle. */
export function claimQuestReward(pid, tier, questIndex) {
  const reward = peekQuestReward(pid, tier, questIndex);
  if (reward > 0) commitQuestClaim(pid, tier, questIndex);
  return reward;
}

/** Peek tier bonus without marking claimed. */
export function peekTierBonus(pid, tier) {
  const t = TIERS[tier];
  const data = getQuestData(pid, tier);
...
/** Commit tier bonus — mark as claimed. */
export function commitTierBonus(pid, tier) {
  const data = getQuestData(pid, tier);
...
/** Legacy: claim + mark in one step. */
export function claimTierBonus(pid, tier) {
  const reward = peekTierBonus(pid, tier);
  if (reward > 0) commitTierBonus(pid, tier);
  return reward;
}
