...
import {
  ...
  syncPlayerData, getGemFromScoreboard, cleanupSyncCooldown,
  getDiscCodes, saveDiscCodes, hasUsedCode, validateDisc, consumeDisc, validateAndConsumeDisc, refundDisc,
...
import { maybeGrandfatherFirstTopup, markOfflineDelivered as _markOfflineDelivered } from "../leaderboard/sync_topup.js";
...
const K_TOPUP_LOG     = "gacha:topup_log";
const TOPUP_LOG_MAX   = 100;
...
let _topupCache = null;
let _topupDirty = false;

function _ensureTopupCache() {
  if (_topupCache === null) _topupCache = dpGetChunked(K_TOPUP_LOG, []);
  return _topupCache;
}

function _flushTopupLog() {
  if (!_topupDirty) return;
  try {
    let log = _topupCache;
    if (!log) return;
    if (log.length > TOPUP_LOG_MAX) { log = log.slice(log.length - TOPUP_LOG_MAX); _topupCache = log; }
    dpSetChunked(K_TOPUP_LOG, log);
    _topupDirty = false;
...
function reveal1x(container, item, key, player, type, chestLoc) {
  ...
  setSlot(container, key, SLOT.C, itemId,
    item.isDup ? `${color}${item.name} §8[§7Dup§8]§7` : `${color}${item.name}`
  );
...
async function showParticleSession(player) {
  while (true) {
    ...
    body += `\n${HR}`;
    const ptHist = dpGet(CFG.K_HIST + player.id, []).filter(h => h.t === 'PT');
    if (ptHist.length) {
      body += `\n§e5 Terakhir:\n`;
      ptHist.slice(0, 5).forEach((h, i) => { body += ` §f${i+1}. ${R[h.r]?.color ?? '§f'}[${h.r?.[0] ?? '?'}] §f${h.n}${h.d ? ' §7(D)' : ''}\n`; });
    }
...
async function showEquipmentSession(player) {
  while (true) {
    ...
    if (free < 1) body += `\n§c⚠ Inventory penuh!`;
    body += `\n${HR}`;
    const eqHist = dpGet(CFG.K_HIST + player.id, []).filter(h => h.t === 'EQ');
    if (eqHist.length) {
      body += `\n§e5 Terakhir:\n`;
      eqHist.slice(0, 5).forEach((h, i) => { body += ` §f${i+1}. ${R[h.r]?.color ?? '§f'}[${h.r?.[0] ?? '?'}] §f${h.n}${h.d ? ' §7(D)' : ''}\n`; });
    }
    ...
    if (action === "claim") {
      ...
      n > 0
        ...
        : player.sendMessage(pend.length ? "§8[§cX§8]§c Inventory penuh!" : "§7Tidak ada pending.");
      continue;
    }
...
async function executeGachaIntent(player, intent, block) {
  ...
  if (!chestBlock || !isValidChest(chestBlock)) {
    sfx(player, SFX.BROKE);
    player.sendMessage(
      `§8[§cGacha§8]§c ⚠ Tidak ada chest gacha terdaftar di dekatmu!\n` +
  ...
  if (!fresh()) {
    dpDel(sessRefKey);
    await withLock(player.id, async () => refund(type, player, cost));
    // [FIX] Refund kode diskon jika chest hilang sebelum sesi dimulai
    if (disc && pendingDisc.has(player.id)) {
      try { refundDisc(disc.code, player.id, { pct: disc.pct, type: disc.type }); } catch {}
      pendingDisc.delete(player.id);
    }
    player.sendMessage(`§8[§cGacha§8]§c ⚠ Chest hilang! §f${cost} ${unit} dikembalikan.`);
  ...
  try {
    ...
    // [FIX BUG-4] Kode diskon sudah dikonsumsi di showDiscountInput (atomic).
    // Di sini cukup hapus dari pending RAM.
    if (disc) {
      pendingDisc.delete(player.id);
    }
    ...
    // [FIX DOUBLE-REFUND] Cek pendingDisc.has() dulu — jika playerLeave sudah
    // me-refund dan delete, jangan refund lagi (mencegah double refund exploit).
    if (disc && pendingDisc.has(player.id)) {
      try {
        refundDisc(disc.code, player.id, { pct: disc.pct, type: disc.type });
        pendingDisc.delete(player.id);
        console.warn(`[Gacha] Kode diskon "${disc.code}" di-refund untuk ${player.name} (gacha gagal).`);
      } catch (e) { console.error("[Gacha] refundDisc error:", e); }
    }
    ...
    if (err.message === "timeout") {
      ...
      player.sendMessage(`§8[§cGacha§8]§c ⚠ Waktu habis! §f${cost} ${unit} dikembalikan.`);
    } else {
      console.error("[Gacha] Error:", err);
      player.sendMessage(`§8[§cGacha§8]§c ⚠ Error! §f${cost} ${unit} dikembalikan.`);
    }
  } finally {
    dpDel(sessRefKey);
    clearLock(key);
...
async function showDiscountInput(player, gachaType) {
  ...
  if (!raw) {
    if (cur) {
      // [FIX BUG-4] Jika player hapus kode yang sudah dikonsumsi, refund uses-nya
      refundDisc(cur.code, player.id, { pct: cur.pct, type: cur.type });
      pendingDisc.delete(player.id);
      player.sendMessage("§8[§7Diskon§8]§7 Kode dihapus & dikembalikan.");
    }
    return;
  }
  // Jika memasukkan kode yang sama dengan yang sudah aktif, abaikan saja
  if (cur && cur.code === raw) { player.sendMessage(`§8[§7Diskon§8]§7 Kode §f${raw} §7sudah aktif.`); return; }

  // Cek apakah sudah pernah pakai kode ini sebelumnya (bukan yang sedang pending)
  if (hasUsedCode(player.id, raw) && (!cur || cur.code !== raw)) {
    player.sendMessage(`§8[§cX§8]§c Kamu sudah memakai kode §f"${raw}"§c.`); return;
  }

  // [FIX BUG-4] Consume atomically di sini, bukan saat pull.
  // Ini mencegah race condition: dua player dengan kode uses=1 tidak bisa keduanya dapat diskon.
  const entry = validateAndConsumeDisc(raw, gachaType, player.id);
  if (!entry) { player.sendMessage(`§8[§cX§8]§c Kode §f"${raw}"§c tidak valid atau habis.`); return; }

  // Jika player sudah punya kode lain yang belum dipakai, refund kode lama
  if (cur) {
    refundDisc(cur.code, player.id, { pct: cur.pct, type: cur.type });
  }
...
async function showAdminMenu(player) {
  ...
  while (true) {
    ...
    const form = new ActionFormData().title("§c  ADMIN PANEL  §r")
      ...
      .button(` Log Topup Terakhir\n§7${_ensureTopupCache().length} catatan`)
    ...
    const res = await form.show(player);
    if (res.canceled) return;
    if (res.selection === 9) return;
    if      (res.selection === 0) await showAdminPlayerSelect(player, "gem");
    else if (res.selection === 1) await showAdminPlayerSelect(player, "coin");
    else if (res.selection === 2) await showAdminTopupLog(player);
    else if (res.selection === 3) await showAdminCreateCode(player);
    else if (res.selection === 4) await showAdminDeleteCode(player);
    else if (res.selection === 5) await showAdminListCodes(player);
    else if (res.selection === 6) await showExportImportAllUI(player);
    else if (res.selection === 7) await showAdminRegisterChest(player);
    else if (res.selection === 8) await showAdminManageChests(player);
  }
}

function _fmtTime(ts) {
  const d = new Date(ts);
  const dd = d.getDate(), mm = d.getMonth() + 1, hh = d.getHours(), mn = d.getMinutes();
  return `${dd < 10 ? "0" : ""}${dd}/${mm < 10 ? "0" : ""}${mm} ${hh < 10 ? "0" : ""}${hh}:${mn < 10 ? "0" : ""}${mn}`;
}

async function showAdminTopupLog(adminPlayer) {
  if (!adminPlayer.hasTag(CFG.ADMIN_TAG)) { adminPlayer.sendMessage("§c[!] Akses ditolak."); return; }

  const PP = 15;
  let page = 0, fCur = null, fAdm = null;

  while (true) {
    const log = _ensureTopupCache();
    const len = log.length;
    let filtered, fLen;
    if (!fCur && !fAdm) { filtered = log; fLen = len; }
    else {
      filtered = [];
      for (let i = 0; i < len; i++) {
        const e = log[i];
        if (fCur && e.c !== fCur) continue;
        if (fAdm && e.a !== fAdm) continue;
        filtered.push(e);
      }
      fLen = filtered.length;
    }

    const totalPages = Math.max(1, Math.ceil(fLen / PP));
    if (page >= totalPages) page = totalPages - 1;
    const start = fLen - 1 - page * PP;
    const end = Math.max(-1, start - PP);

    const admSet = new Set();
    for (let i = 0; i < len; i++) admSet.add(log[i].a);
    const admArr = [...admSet];

    let body = `${HR}\n§c⚡ LOG TRANSAKSI ADMIN\n§7Total: §f${len}  §7Tampil: §f${fLen}`;
    if (fCur) body += `  §7Filter: §e${fCur}`;
    if (fAdm) body += `  §7Admin: §a${fAdm}`;
    body += `\n${HR}\n`;

    if (start < 0) {
      body += "§7(Belum ada transaksi tercatat)\n";
    } else {
      for (let i = start; i > end; i--) {
        const e = filtered[i];
        const ac = e.x === "add" ? "§a+" : e.x === "remove" ? "§c-" : "§e=";
        const cc = e.c === "gem" ? "§b" : "§e";
        body += `§8${_fmtTime(e.ts)} §a${e.a}\n  ${ac}${e.x.toUpperCase()} ${cc}${e.n} ${e.c}§f → ${e.t}${e.o ? " §8[§7OFF§8]§7" : ""}\n  §7Sblm: §f${e.b}  §7Ssdh: §f${e.f}\n`;
      }
    }
    body += `${HR}\n§7Halaman §f${page + 1}§7/§f${totalPages}`;

    const form = new ActionFormData().title("§c  LOG TOPUP ADMIN  §r").body(body);
    form.button(page > 0 ? "§a◀ Sebelumnya" : "§8◀ Sebelumnya");
    form.button(page < totalPages - 1 ? "§a▶ Berikutnya" : "§8▶ Berikutnya");
    form.button(fCur ? `§e⚙ Filter: ${fCur}` : "§f⚙ Filter Mata Uang");
    form.button(fAdm ? `§a⚙ Admin: ${fAdm}` : "§f⚙ Filter Admin");
    form.button(" Kembali");
    ...
    if (sel === 0) { if (page > 0) page--; continue; }
    if (sel === 1) { if (page < totalPages - 1) page++; continue; }
    if (sel === 2) {
      fCur = fCur === null ? "gem" : fCur === "gem" ? "coin" : null;
      page = 0; continue;
    }
    if (sel === 3) {
      if (!admArr.length) continue;
      if (fAdm === null) fAdm = admArr[0];
      else { const idx = admArr.indexOf(fAdm); fAdm = idx < admArr.length - 1 ? admArr[idx + 1] : null; }
      page = 0; continue;
    }
    if (sel === 4) return;
  }
...
async function showAdminPlayerSelect(adminPlayer, currency) {
  while (true) {
    ...
    for (const p of allPlayers) {
      ...
      const pendG   = dpGet(K_PEND_GEM  + p.id, null) !== null ? " §8[§epend§8]§e" : "";
      const pendC   = dpGet(K_PEND_COIN + p.id, null) !== null ? " §8[§epend§8]§e" : "";
      form.button(
...
async function showAdminAction(adminPlayer, target, currency) {
  ...
  while (true) {
    const live = world.getPlayers().find(p => p.id === targetId);
    if (!live) { adminPlayer.sendMessage(`§c[!] §f${targetName} §csudah offline.`); return false; }
...
async function showAdminAmountInput(adminPlayer, target, currency, action) {
  ...
  try {
    const live = world.getPlayers().find(p => p.id === target.id);
    if (!live) { adminPlayer.sendMessage(`§c[!] §f${target.name} §csudah offline.`); return false; }
    const balBefore = isGem ? getGem(live) : getCoin(live);
    const success = await withLock(live.id, async () => {
      if (isGem) {
        if (action === "add")         setGem(live, getGem(live) + amount);
        else if (action === "remove") setGem(live, Math.max(0, getGem(live) - amount));
        else if (action === "set")    setGem(live, amount);
      } else {
        const cur = getCoin(live);
        if (action === "add")         setScore(CFG.COIN_OBJ, live, cur + amount);
        else if (action === "remove") setScore(CFG.COIN_OBJ, live, Math.max(0, cur - amount));
        else if (action === "set")    setScore(CFG.COIN_OBJ, live, amount);
      }
      return true;
    });
    if (success) {
      const newBal = isGem ? getGem(live) : getCoin(live);
      recordTopupLog(adminPlayer.name, live.name, live.id, currency, action, amount, balBefore, newBal, false);
      adminPlayer.sendMessage(`§8[§aAdmin§8]§a ${actLbl} §f${amount} ${currency} -> §a${live.name}§f. Baru: §e${newBal}`);
      sfx(adminPlayer, SFX.ADMIN);
      live.sendMessage(`§a[+] Saldo ${currency} diperbarui: §e${newBal}`);
      if (isGem) syncPlayerData(live);
      return true;
    }
    adminPlayer.sendMessage("§8[§cAdmin§8]§c Gagal. Coba lagi."); return false;
  } catch (e) {
    adminPlayer.sendMessage(`§8[§cAdmin§8]§c Error: target mungkin sudah offline.`);
    return false;
  }
}
...
async function showAdminActionOffline(adminPlayer, target, currency, reg) {
  ...
  recordTopupLog(adminPlayer.name, target.name, target.id, currency, action, amount, curBal, newBal, true);

  adminPlayer.sendMessage(
    `§8[§aAdmin§8]§a ${actLbl} §f${amount} ${currency} -> §7○§f${target.name}§f. Baru: §e${newBal}\n§7(Diterapkan saat login berikutnya)`
  );
  sfx(adminPlayer, SFX.ADMIN);
  return true;
}
...
async function showAdminCreateCode(adminPlayer) {
  const res = await new ModalFormData().title("§a  Buat Kode Diskon  §r")
    ...
    .slider("§fDiskon (%)", 5, 90, { valueStep: 5, defaultValue: 50 })
    .dropdown("§fBerlaku untuk", ["Semua Gacha", "Partikel Saja", "Peralatan Saja"], { defaultValueIndex: 0 })
    .slider("§fJumlah Pakai", 1, 50, { valueStep: 1, defaultValue: 1 }).show(adminPlayer);
  if (res.canceled) return;
  ...
  // [GUARD] Validasi numeric values dari form (defensive)
  const pctNum = Number(discPct);
  const usesNum = Math.floor(Number(uses) || 1);
  if (!Number.isFinite(pctNum) || pctNum < 1 || pctNum > 99) {
    adminPlayer.sendMessage("§c[!] Diskon harus 1-99%."); return;
  }
  if (!Number.isFinite(usesNum) || usesNum < 1 || usesNum > 1000) {
    adminPlayer.sendMessage("§c[!] Jumlah pakai harus 1-1000."); return;
  }
  const tIdx = Math.max(0, Math.min(2, Math.floor(Number(typeIdx) || 0)));
  ...
  map[code] = { pct: pctNum, type: typeMap[tIdx], uses: usesNum };
  saveDiscCodes(map);
  adminPlayer.sendMessage(`§8[§aAdmin§8]§a Kode §f${code} §adibuat! Diskon §f${pctNum}%% §a| §f${typeLbl[tIdx]} §a| §f${usesNum}x`);
  sfx(adminPlayer, SFX.ADMIN);
}
...
async function showAdminDeleteCode(adminPlayer) {
  ...
  if (!confirm.canceled && confirm.selection === 1) {
    const fresh = getDiscCodes();
    delete fresh[targetCode];
    saveDiscCodes(fresh);

    // [FIX] Clear semua pendingDisc untuk kode ini agar tidak di-recreate via refundDisc
    // Players yang sedang punya kode pending akan kehilangan diskon, tapi tidak kehilangan saldo
    // (karena gacha belum execute saat ini hanya kode yang dipending sebelum bayar).
    let cleared = 0;
    for (const [pid, disc] of pendingDisc) {
      if (disc && String(disc.code).toUpperCase() === targetCode) {
        pendingDisc.delete(pid);
        cleared++;
        // Optional: notify player jika online
        try {
          const p = world.getPlayers().find(pl => pl.id === pid);
          if (p) p.sendMessage(`§8[§7Diskon§8]§e ⚠ Kode §f${targetCode}§e dihapus admin. Pilih kode lain atau lanjut tanpa diskon.`);
        } catch {}
      }
    }
    if (cleared > 0) {
      adminPlayer.sendMessage(`§8[§aAdmin§8]§a Kode §f${targetCode} §adihapus. §7(${cleared} player pending di-clear)`);
    } else {
      adminPlayer.sendMessage(`§8[§aAdmin§8]§a Kode §f${targetCode} §adihapus.`);
    }
    sfx(adminPlayer, SFX.ADMIN);
  }
}
  ...
  if (!block) {
    await new ActionFormData()
      ...
      .body(
        ...
        `§7Langsung §aklik §7ke chest yang ingin didaftarkan.\n` +
        `§7Sistem akan otomatis membuka form registrasi.\n\n` +
...
async function showAdminManageChests(adminPlayer) {
  ...
  while (true) {
    ...
    for (const c of allowed) {
      const typeLbl = c.type === "PARTICLE" ? "§8[§5PT§8]§5" : "§8[§6EQ§8]§6";
      const inUse   = activeChests.has(c.key) ? " §8[§cAKTIF§8]§c" : "";
    ...
    if (!confirm.canceled && confirm.selection === 1) {
      ...
      adminPlayer.sendMessage(
        `§8[§aAdmin§8]§a Chest dihapus dari whitelist.\n§7${target.label} @ ${target.x}, ${target.y}, ${target.z}`
      );
      sfx(adminPlayer, SFX.ADMIN);
    }
  }
...
async function showExportImportAllUI(adminPlayer) {
  ...
  while (true) {
    ...
    const form = new ActionFormData().title("§3  Export / Import  §r")
      ...
      .button(" Export Data")
      .button(hasStagedImport ? "§c Import Data §8[§eSIAP ⚡§8]§e" : " Import Data")
    ...
    if      (res.selection === 0) await showBulkExportUI(adminPlayer);
      ...
      if (!confirm.canceled && confirm.selection === 1) {
        ...
        adminPlayer.sendMessage("§8[§aAdmin§8]§a Data staged dihapus.");
        sfx(adminPlayer, SFX.ADMIN);
      }
    }
  }
...
async function showBulkImportUI(adminPlayer) {
  ...
  if (step2.canceled || step2.selection !== 1) {
    adminPlayer.sendMessage("§8[§7Import§8]§7 Dibatalkan.");
    return;
  }

  adminPlayer.sendMessage("§8[§eGacha§8]§e Menerapkan import, mohon tunggu...");
...
async function showPendingImportList(adminPlayer) {
  ...
  if (!confirm.canceled && confirm.selection === 1) {
    ...
    adminPlayer.sendMessage(`§8[§aAdmin§8]§a Pending import §f${target.name}§a dibatalkan.`);
    sfx(adminPlayer, SFX.ADMIN);
  }
}
...
world.afterEvents.playerSpawn.subscribe(({ player, initialSpawn }) => {
  if (initialSpawn) {
    ...
    // [Migration] Grandfather player yang sudah pernah engage gem economy
    // (saldo gem > 0 atau punya particle) supaya tidak double-claim
    // first-topup bonus saat migrasi ke world baru. Idempotent — sekali
    // per player per world. Wajib SETELAH gem & particle tag di-restore.
    try {
      const ptMask = playerPtMask(player);
      const gemBal = getGem(player);
      maybeGrandfatherFirstTopup(player.name, gemBal, ptMask);
    } catch (e) { console.warn("[Gacha] grandfather check error:", e); }
  }
  ...
  system.runTimeout(() => {
    ...
    if (pendGem !== null) {
      try {
        const currentBal = getGem(live);
        const reg = getPlayerReg();
        const regBal = reg[live.id]?.gem ?? 0;
        let newBal;
        if (pendGem !== regBal) {
          // Web topup: registry NOT updated → compute delta and ADD to current balance
          const delta = Math.max(0, pendGem - regBal);
          newBal = currentBal + delta;
          console.log(`[Topup] Deliver gem DELTA: current=${currentBal} regBal=${regBal} pend=${pendGem} delta=${delta} → ${newBal}`);
        } else {
          // Admin SET (registry was updated to match pending) → use absolute value
          newBal = pendGem;
          console.log(`[Topup] Deliver gem ABSOLUTE: current=${currentBal} pend=${pendGem} → ${newBal}`);
        }
        setGem(live, newBal);
        dpDel(K_PEND_GEM + live.id);
        const gained = newBal - currentBal;
        live.sendMessage(`§a[+] §b${gained > 0 ? "+" + gained + " " : ""}Gem §7ditambahkan oleh admin. Saldo: §b${newBal}`);
      } catch (e) { console.warn("[Gacha] pending gem delivery error:", e); }
      _markOfflineDelivered(live.name, "gem");
    }
    const pendCoin = dpGet(K_PEND_COIN + live.id, null);
    if (pendCoin !== null) {
      try {
        const currentCoin = getCoin(live);
        const reg = getPlayerReg();
        const regCoin = reg[live.id]?.coin ?? 0;
        let newCoin;
        if (pendCoin !== regCoin) {
          const delta = Math.max(0, pendCoin - regCoin);
          newCoin = currentCoin + delta;
        } else {
          newCoin = pendCoin;
        }
        setScore(CFG.COIN_OBJ, live, newCoin);
        dpDel(K_PEND_COIN + live.id);
        const gained = newCoin - currentCoin;
        live.sendMessage(`§a[+] §e${gained > 0 ? "+" + gained + " " : ""}Koin §7ditambahkan oleh admin. Saldo: §e${newCoin}`);
      } catch (e) { console.warn("[Gacha] pending coin delivery error:", e); }
      _markOfflineDelivered(live.name, "coin");
    }
  ...
  }, 180); // was 100 — stagger after leaderboard(140)
  system.runTimeout(() => syncPlayerData(player), 220); // was 120
  ...
  }, 120); // was 60 — stagger after daily(80)
});
...
world.afterEvents.playerLeave.subscribe(({ playerId }) => {
  ...
  // [FIX] Refund kode diskon jika player leave sebelum pull
  const disc = pendingDisc.get(playerId);
  if (disc) {
    try { refundDisc(disc.code, playerId, { pct: disc.pct, type: disc.type }); } catch {}
  }
  pendingDisc.delete(playerId);
  chestCache.delete(playerId);
  lastActionBar.delete(playerId);
  pendingChestInteract.delete(playerId);
  cleanupSyncCooldown(playerId); // [PERF] prevent _syncCooldown leak
...
// ═══════════════════════════════════════════════════════════
// COMMAND REGISTRATION — /gacha (buka Hub Gacha)
// ═══════════════════════════════════════════════════════════
      ...
      (origin) => {
        ...
        system.run(async () => {
          try {
            ...
            if (isFeatureOnNPC("gacha")) {
              player.sendMessage("§8[Sistem]§c Fitur ini tersedia lewat NPC.\n§7Kunjungi NPC di spawn untuk mengakses Gacha.");
              return;
            }
          } catch {}
          // Fallback: buka langsung
          openGachaNPC(player).catch(() => {});
        });
        return { status: 0 };
      }
    );
  } catch (e) { console.warn("[Gacha] Command registration failed:", e); }
});
...
// ═══════════════════════════════════════════════════════════
// NPC MARKET ENTRY — exported for npc/market.js
// ═══════════════════════════════════════════════════════════

/** NPC Market entry — called from npc/market.js */
export async function openGachaNPC(player) {
  if (isPurgeActive()) {
    player.sendMessage("§8[Sistem] §cPilar Energi offline — gacha tidak tersedia.");
    return;
  }
  ...
  if ((lastPull.get(player.id) ?? 0) + CFG.PULL_CD > system.currentTick) {
    player.sendMessage("§8[§eGacha§8]§e Tunggu sebentar!");
    return;
  }
