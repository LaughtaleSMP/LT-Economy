...
const DEF_STATS = () => ({ total: 0, by: Object.fromEntries(R_KEYS.map(k => [k, 0])) });

export function getStats(key, p) {
  const r = dpGet(key + p.id, null);
  if (!r) return DEF_STATS();
  return { total: r.total ?? 0, by: r.by ?? DEF_STATS().by };
}
...
export function pushGlobalHist(playerName, items, type) {
  ...
  for (const item of toRecord)
    hist.unshift({ p: playerName, t: isPt ? "PT" : "EQ", r: item.rarity, n: item.name, ts: Date.now() });
