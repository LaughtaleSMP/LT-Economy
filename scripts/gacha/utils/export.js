...
import { getKillFx, setKillFx } from "../../kill_fx.js";
...
export function buildExportString(player) {
  ...
  // Include owned kill effects (exclude defaults)
  const kfx = getKillFx(player.id);
  ...
  return `${EXPORT_VER}|gem:${getGem(player)}|pt:${tags}|eqsr:${eq.sr}|eql:${eq.l}|kfx:${kfxStr}`;
}
...
export function parseImportString(str) {
  try {
    ...
    let gem = 0, particles = [], eqsr = 0, eql = 0, kfxOwned = [];
    for (const part of parts.slice(1)) {
      ...
      if (key === "kfx" && val) {
        // Parse kill effect IDs — may contain JSON arrays
        kfxOwned = val.split(",").reduce((acc, chunk) => {
          if (chunk.startsWith("[")) {
            acc._buf = chunk;
          } else if (acc._buf) {
            acc._buf += "," + chunk;
            if (chunk.endsWith("]")) {
              try { acc.push(JSON.parse(acc._buf)); } catch { acc.push(acc._buf); }
              acc._buf = null;
            }
          } else {
            acc.push(chunk);
          ...
          return acc;
        }, Object.assign([], { _buf: null }));
        delete kfxOwned._buf;
      }
    }
    return { ok:true, gem, particles, eqsr, eql, kfxOwned };
...
export function applyImport(target, data) {
  ...
  if (data.kfxOwned && data.kfxOwned.length > 0) {
    const kfx = getKillFx(target.id);
    for (const eid of data.kfxOwned) {
      const _idKey = (id) => Array.isArray(id) ? JSON.stringify(id) : id;
      if (!kfx.owned.some(o => _idKey(o) === _idKey(eid))) {
        kfx.owned.push(eid);
      }
    }
    setKillFx(target.id, kfx);
  }
  syncPlayerDataForce(target);
}

export function applyImportOffline(playerId, data) {
  dpSet(CFG.K_IMPORT_PEND + playerId, { gem: data.gem, particles: data.particles, eqsr: data.eqsr, eql: data.eql, kfxOwned: data.kfxOwned || [] });
  dpSet(CFG.K_EQ_PITY + playerId, { sr: Math.min(data.eqsr, CFG.EQ_PITY_RARE), l: Math.min(data.eql, CFG.EQ_PITY_LEG) });
  // Sync p_reg so offline export stays accurate without pt_d:
  const reg = getPlayerReg();
  if (reg[playerId]) {
